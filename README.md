...
* AWS account and CLI working as described [here](https://aws.amazon.com/cli/)
...
To generate wireguard's public/private encryption keys, you will also need the wireguard `wg` tools:
...
From within the terraform directory, copy the file `terraform.tfvars.examples` to `terraform.tfvars` and 
...
* replace the file with a new file on the server and delete the original
